...
import { EASE_ACCEL, enterOnce, fxEnter, fxOut, softSpring } from "~/lib/motion";
      ...
      {items.map((it, i) => (
        <button
          ...
          ref={(el: Element | undefined) => {
            enterOnce(el, { x: -10, duration: 0.3, delay: Math.min(i * 0.03, 0.3) });
          }}
          class={cn(
        ...
        >
          <div
            class={cn(
              ...
              current === it.id ? "text-on-secondary-container" : "text-on-surface",
            )}
          >
          ...
          <div
            class={cn(
              ...
              current === it.id ? "text-on-secondary-container/75" : "text-on-surface-variant",
            )}
          >
            {it.preview ?? ""}
          </div>
  ...
  protected override render() {
    return (
      <aside class="border-outline-variant bg-surface-container-low flex h-full w-64 shrink-0 flex-col border-r">
        <div class="text-title-small text-on-surface flex items-center gap-2 px-4 pt-4 pb-3">History</div>
...
export class MobileDrawer extends LightElement {
  ...
  private async close(): Promise<void> {
    ...
    if (panel) {
      outs.push(animate(panel, { x: [0, -320] }, { duration: 0.25, ease: EASE_ACCEL }).then(() => undefined));
    }
  ...
  protected override render() {
    ...
    return (
      ...
      >
        <div
          ref={this.panelRef}
          class="bg-surface-container-low shadow-e5 flex h-full w-[300px] flex-col rounded-r-xl"
          role="dialog"
