...
export interface TicketDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * 
   * // Only select the `ticketNo`
   * const ticketWithTicketNoOnly = await prisma.ticket.findMany({ select: { ticketNo: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many Tickets and only return the `ticketNo`
   * const ticketWithTicketNoOnly = await prisma.ticket.createManyAndReturn({
   ...
   * })
   * 
   * // Update zero or more Tickets and only return the `ticketNo`
   * const ticketWithTicketNoOnly = await prisma.ticket.updateManyAndReturn({
   *   select: { ticketNo: true },
   *   where: {
   *     // ... provide filter here
   *   },
