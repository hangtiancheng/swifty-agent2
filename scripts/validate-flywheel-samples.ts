// ch09 prompt labeled-sample validation (working requirement: for a pure-prompt task, run the
...
async function main(): Promise<number> {
  ...
  for (const s of samples) {
    ...
    try {
      ...
      console.log(`✗ ${s.raw_question.slice(0, 24)}… call failed ${error instanceof Error ? error.constructor.name : "Error"}`);
      continue;
    }
    const matchOk = r.matched_question_id === s.expect_match;
    const kwOk = s.expect_keywords.every((k) => r.normalized_question.includes(k));
