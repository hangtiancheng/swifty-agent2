// Intent four-piece labeled evaluation: nine-class accuracy (ch08 added human_agent) +
// confidence parseable + weird utterances fall to other + multi-turn drift. Requires chat upstream.
// Run: node scripts/eval-intent.ts
...
const SAMPLES: [string, string][] = [
  ...
  ["The litter box is leaking electricity; create a ticket to follow up", "human_agent"],
  ["Hi there", "chitchat"],
...
async function main(): Promise<void> {
  ...
  for (const [q, expect] of SAMPLES) {
    ...
    const ok = got === expect;
    const jsonOk = VALID.has(got) && typeof conf === "number" && conf >= 0 && conf <= 1;
    badJson += jsonOk ? 0 : 1;
    passed += ok ? 1 : 0;
    console.log(`${ok ? "✅" : "❌"} ${JSON.stringify(q)} -> ${got} (conf=${conf}) expected=${expect}`);
  }
  ...
  console.log(`Multi-turn drift "So where is it now" (after refund) -> ${r.intent} (expected logistics)`);
