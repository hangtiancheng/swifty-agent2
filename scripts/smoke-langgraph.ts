// ch05 red-line smoke: does StateGraph + a checkpointer + multi-mode streaming work on the current
...
async function main(): Promise<void> {
  ...
  const config = { configurable: { thread_id: "smoke-1" }, streamMode: streamModes };
  const seenModes = new Set<string>();
  const stream = await graph.stream({ messages: [new HumanMessage("Introduce yourself in one sentence")] }, config);
  for await (const chunk of stream) {
    ...
    if (mode === "messages") {
      ...
      if (parsed.success) {
        ...
        const text = BaseMessage.isInstance(msg) ? contentToString(msg.content) : "";
        console.log("MSG node=", meta.langgraph_node, "text=", text.slice(0, 20));
      }
    } else {
      const update = updatesChunkSchema.safeParse(payload);
      console.log("UPD", update.success ? Object.keys(update.data) : "(unparseable)");
    }
  }
