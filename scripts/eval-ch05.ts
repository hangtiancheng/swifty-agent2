// ch05 end-to-end acceptance evaluation. Requires the full stack running
// (DB + MCP servers + chat upstream + this app). Run: node scripts/eval-ch05.ts
...
const agentResponseSchema = z.object({
  ...
  suggested_actions: z.array(z.object({ type: z.string() }).loose()).default([]),
...
async function agent(message: string): Promise<AgentResponse> {
  ...
  const resp = await fetch(`${BASE}/api/agent`, {
    ...
    body: JSON.stringify({ user_id: "eval-ch05", message, conversation_id: null }),
...
async function main(): Promise<void> {
  ...
  const b5 = await agent("Has the order ending in 1001 shipped? Where is it now?");
  ...
  console.log("=".repeat(60));
  console.log('Acceptance 1 (the forced-retrieval node is reached): ask "What is the returns policy", then check the app logs;');
  console.log("  you should see `turn ... route=knowledge trace={... forced_rag: true ...}`");
  console.log('Acceptance 3 (frontend): in the browser, clicking "transfer to human" shows transferred + a greeting from the service cat;');
  console.log('  clicking "create ticket" writes into tickets; clicking neither keeps the conversation going normally');
