...
async function runAgent(query: string, maxTurns = 6): Promise<string> {
  ...
  const messages: BaseMessage[] = [new SystemMessage(AGENT_SYSTEM), new HumanMessage(query)];
  for (let step = 1; step <= maxTurns; step += 1) {
    ...
    for (const tc of toolCalls) {
      console.log(`[step ${step}] calling tool ${tc.name} args=${JSON.stringify(tc.args)}`);
      const run = await engine.executeToolCall({ name: tc.name, id: tc.id ?? "", args: tc.args }, 0, specs);
      messages.push(run.toolMessage);
    }
  }
  console.log(`[cap] max_turns=${maxTurns} exhausted without converging; production should fall back here`);
