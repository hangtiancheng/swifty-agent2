...
const CASES: [string, string, "rewrite" | "passthrough"][] = [
  ["user: When will the Bluetooth earphones arrive\nassistant: Expected to be delivered tomorrow", "Can this be returned", "rewrite"],
  ["user: The cat food from order 1001\nassistant: Shipped", "Where is it now", "rewrite"],
...
async function main(): Promise<void> {
  for (const [hist, q, kind] of CASES) {
    ...
    const changed = got !== q;
    const ok = (changed && kind === "rewrite") || (!changed && kind === "passthrough");
    console.log(`${ok ? "✅" : "❌"} ${JSON.stringify(q)} -> ${JSON.stringify(got)} expected=${kind}`);
  }
  console.log(
