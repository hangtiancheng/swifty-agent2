// ch08 acceptance end-to-end evaluation: ticket-confirm flow (follow-up / preview / confirm /
// cancel) + logistics served by the MCP server. Requires the full stack running
// (DB + both MCP servers + chat upstream + this app). Run: node scripts/eval-ch08.ts
...
const agentResponseSchema = z.object({
  ...
  interrupt: z
    .object({
      ...
      preview: z.object({ description: z.string().optional() }).loose().optional(),
    })
    .loose()
    .nullable(),
});
...
async function agent(message: string, conversationId: number | null = null): Promise<AgentResponse> {
  const resp = await fetch(`${BASE}/api/agent`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ user_id: "eval-ch08", message, conversation_id: conversationId }),
...
async function resume(conversationId: number, confirmed: boolean): Promise<string> {
...
async function latestTicketNo(): Promise<string | null> {
  const row = await prisma.ticket.findFirst({ orderBy: [{ createdAt: "desc" }, { ticketNo: "desc" }] });
  return row?.ticketNo ?? null;
}

async function latestAuditStatus(toolName: string): Promise<string | null> {
  const row = await prisma.toolAuditLog.findFirst({ where: { toolName }, orderBy: { id: "desc" } });
...
async function main(): Promise<void> {
  ...
  const r2 = await agent("My smart litter box is leaking electricity; it trips the breaker as soon as it powers on", cid);
  const intr = r2.interrupt;
  const okPreview = intr !== null && intr.type === "confirm_ticket" && Boolean(intr.preview?.description);
  results.push({
  ...
  results.push({
    ...
    ok: ticketsAfter === ticketsBefore + 1 && auditOk === "success" && ticketNo !== null && answer.includes(ticketNo ?? ""),
  ...
  const r3 = await agent("Please create a ticket for me; the third level of my cat tree collapsed");
  ...
  results.push({
    ...
    ok: intr3 !== null && intr3.type === "confirm_ticket" && ticketsFinal === ticketsAfter && auditDeny === "permission_denied",
  ...
  const enStatus = ["Picked up", "In transit", "Out for delivery", "Delivered"].some((s) =>
    (lg[0]?.content ?? "").includes(s),
  );
  ...
  results.push({
    ...
    ok: names.includes("query_logistics") && enStatus && src === "mcp/logistics",
